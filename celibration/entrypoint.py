...
def run(args):
    ...
    try:
        ...
        config_path = Path(celibration.__file__).parents[0] / "celibration_config.yaml"
        plugins_folder = config_path.resolve()
    except ModuleNotFoundError as err:
        raise f"Something went wrong, see {err}"

    manager = CalibrationPlanManager(config=config_path)
    if args.verbosity:
